...
class TestShiftImageAnchors:
    ...
    def test_one_cell_anchor_shifts_below_insert(self):
        ...
        ws.add_image(img, "A7")  # row 7 (1-based) → _from.row = 6 (0-based)
    ...
    def test_images_above_insert_untouched(self):
        ...
        ws = wb.active
        img = XlImage(io.BytesIO(_PNG_DATA))
        ws.add_image(img, "A2")  # row 2 (1-based) → _from.row = 1 (0-based)
    ...
    def test_zero_rows_noop(self):
        ...
        ws = wb.active
        img = XlImage(io.BytesIO(_PNG_DATA))
        ws.add_image(img, "A5")  # row 5 (1-based) → _from.row = 4 (0-based)
    ...
    def test_multiple_images_all_shift(self):
        ...
        img1 = XlImage(io.BytesIO(_PNG_DATA))
        img2 = XlImage(io.BytesIO(_PNG_DATA))
        img3 = XlImage(io.BytesIO(_PNG_DATA))

        ws.add_image(img1, "A8")   # row 8  → 0-based row 7
        ws.add_image(img2, "A10")  # row 10 → 0-based row 9
        ws.add_image(img3, "A12")  # row 12 → 0-based row 11
    ...
    def test_survives_save_reload(self):
        ...
        ws = wb.active
        img = XlImage(io.BytesIO(_PNG_DATA))
        ws.add_image(img, "A7")  # row 7 → 0-based row 6
