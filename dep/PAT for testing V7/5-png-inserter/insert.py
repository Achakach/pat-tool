...
def main(config=None):
    if config is None:
        ...
        if not xlsx_folder.is_absolute():
            xlsx_folder = (Path(__file__).parent / xlsx_folder).resolve()
        if not png_folder.is_absolute():
            png_folder = (Path(__file__).parent / png_folder).resolve()
        if not output_folder.is_absolute():
            output_folder = (Path(__file__).parent / output_folder).resolve()
    else:
        matching_file = Path(config["matching_file"]).resolve()
        xlsx_folder = Path(config["xlsx_folder"]).resolve()
        png_folder = Path(config["png_folder"]).resolve()
